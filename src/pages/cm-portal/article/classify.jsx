...
import { message, Tag, Button, Popconfirm, Form, Input, Select, Switch, Radio } from 'antd';
import CmBasePage from '@components/CmBasePage';
import DataForm from '@components/DataForm';
import CmUpload from '@components/CmUpload';
import CmImage from '@components/CmImage';
import { savePArticleCategory, updatePArticleCategory, deletePArticleCategory } from '@api';
...
const ArticleClassifyPage = () => {
  ...
  const handleDelete = async (record) => {
    try {
      const response = await deletePArticleCategory({ id: record.id });
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      ...
      const response = editingRecord
        ? await updatePArticleCategory(submitValues)
        : await savePArticleCategory(submitValues);
  ...
  const loadData = async (params) => {
    try {
      const response = await fetch(`/api/pArticleCategory/getList?${new URLSearchParams(params)}`).then(res => res.json());
      if (response.code === 0 || response.code === 200) {
  ...
  const getInitialValues = () => {
    if (editingRecord) {
      return { state: 1, ...editingRecord };
    }
    if (parentRecord) {
      return { pid: parentRecord.id, state: 1, orderNo: 0 };
    }
    return { pid: '', state: 1, orderNo: 0 };
  };
  ...
  return (
    <div style={{ textAlign: 'left' }}>
      ...
      <DataForm
        key={editingRecord ? `edit-${editingRecord.id}` : formVisible ? 'new' : 'closed'}
        visible={formVisible}
        title={getFormTitle()}
...
export default ArticleClassifyPage;
