...
import { message, Tag, Form, Input, Radio, Switch, TreeSelect } from 'antd';
...
import { savePArticleInfo, updatePArticleInfo, deletePArticleInfo, getPArticleCategoryList } from '@api';
...
const ArticleListPage = () => {
  ...
  const handleDelete = async (record) => {
    try {
      const response = await deletePArticleInfo({ id: record.id });
      if (response.code === 0 || response.code === 200) {
  ...
  const handleSubmit = async (values) => {
    ...
    try {
      const response = editingRecord
        ? await updatePArticleInfo(values)
        : await savePArticleInfo(values);
  ...
  const findClassifyName = (categoryId) => {
    ...
    if (classify) return classify.name;

    // 在子分类中查找
    for (const [, item] of classifyMap) {
      if (item.children) {
  ...
  const handleCategorySelect = (categoryId) => {
    ...
    // 触发表格重新加载，使用与 CmBasePage 相同的选择器
    setTimeout(() => {
  ...
  const loadData = async (params) => {
    try {
      ...
      // 使用 ref 获取最新的 categoryId，确保能获取到即时更新的值
      ...
      const response = await fetch(`/api/pArticleInfo/getList?${new URLSearchParams(requestParams)}`).then(res => res.json());
  ...
  return (
    <div className="article-list-page">
      <div className="article-list-container" style={{ padding: 0 }}>
        {/* 左侧分类树 */}
        {!sidebarCollapsed && (
      ...
      <DataForm
        visible={formVisible}
        title={viewMode ? '查看新闻' : (editingRecord ? '编辑新闻' : '新增新闻')}
