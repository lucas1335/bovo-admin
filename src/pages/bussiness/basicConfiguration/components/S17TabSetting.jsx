...
const S17TabSetting = () => {
  ...
  const getList = async () => {
    setLoading(true);
    try {
      const res = await getSettingConfig('TAB_SETTING');
      if (res.code === 200) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (editingItem) {
        ...
        const res = await saveSettingConfigWithBio('TAB_SETTING', newData);
        if (res.code === 200) {
    ...
    <div>
      ...
      >
        <Form form={form} layout="vertical">
          <Form.Item label="币种" name="name" rules={[{ required: true, message: '币种不能为空' }]}>
            <Input disabled />
          </Form.Item>
